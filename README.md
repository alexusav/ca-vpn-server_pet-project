...
#
#### Backup server. Разворачивание Backup сервера
#

Сервер выполняет роль хранилища конфигурационных файлов, сертификатов и важных файлов с других серверов.
Для работы c сервером и настройки доступа по ключам необходимо запустить скрипт backup-server/confifure_ssh.sh на каждом из серверов, который создаст необходимые ключи. Созданные публичные ключи необходимо скопировать на Backup сервера в файл ~/.ssh/authorized_keys.
На этом конфигурация сервера закончена.

Остальные скрипты из директории backup-server необходимо скопировать соответственно названию на каждый сервер в директорию /usr/local/bin/ и добавить в CRON задачу: 
```
0 2 * * * /usr/local/bin/NAME-create-backup.sh >> /var/log/backup.log 2>&1
```

Скрипты при выполнении реализуют создание зашифрованного архива, который содержит конфиги и другие важные файлы, а затем копирует их на Backup сервер. Так же предусмотрена ротация файлов и хранение копий не старше недели.

Из особенностей можно выделить такие пункты:
- Шифрование бэкапа с помощью GPG (AES256)
